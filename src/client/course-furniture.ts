...
export type FlagPoint = { readonly x: number; readonly z: number }
...
export function buildCheckpointFlags(points: readonly FlagPoint[], tools: MeshTools): THREE.Group {
  ...
  points.forEach((point, index) => {
    scratch.makeTranslation(point.x - 3.4, 1.5, point.z)
    poles.setMatrixAt(index, scratch)
    scratch.makeTranslation(point.x - 3.4 + 0.6, 2.6, point.z)
...
export function buildFinishGate(course: CourseDefinition, tools: MeshTools): FinishGate {
  ...
  const postGeometry = tools.geometry.get(
    "finish-post",
    () => new THREE.CylinderGeometry(0.34, 0.42, 5.4, 16),
  )
  ...
  const beam = new THREE.Mesh(
    tools.geometry.get("finish-beam", () => new RoundedBoxGeometry(9.4, 1.1, 0.9, 3, 0.3)),
    material,
  )
  beam.position.set(
  ...
  const bannerGeometry = new THREE.PlaneGeometry(8.4, 1.5)
